    ...
    def create_single_pdf(self, cursor, uid, ids, data, report_xml, context=None):
        ...
        if report_xml.precise_mode:
            ...
            for obj in self.parser_instance.localcontext['objects']:
                ctx['objects'] = [obj]
                try :
                    html = h.unescape(body_mako_tpl.render(dict(ctx)))
                    htmls.append(html)
                except Exception, e:
                    msg = u"%s" % e
                    _logger.error(msg)
            ...
            try :
                html = h.unescape(body_mako_tpl.render(dict(self.parser_instance.localcontext)))
                htmls.append(html)
            except Exception, e:
                msg = u"%s" % e
                _logger.error(msg)
        ...
        if footer :
            ...
            try :
                foot = foot_mako_tpl.render(dict({}, **self.parser_instance.localcontext))
            except Exception, e:
                msg = u"%s" % e
                _logger.error(msg)
                raise except_osv(_('Webkit render!'), msg)
