...
class ir_actions_report_xml(orm.Model):
    ...
    _columns = {
        'webkit_header':      fields.property('ir.header_webkit', type='many2one', relation='ir.header_webkit', string='Webkit Header', required=True, help="The header linked to the report"),
        ...
        'precise_mode':       fields.boolean('Precise Mode', help="This mode allow more precise element position as each object is printed on a separate HTML but memory and disk usage are wider.")
    }
    ...
    def _lookup_report(self, cr, name):
        ...
        if False and 'report.' + name in openerp.report.interface.report_int._reports:
            ...
            if r:
                ...
                new_report = WebKitParser('report.'+r['report_name'], r['model'], opj('addons',r['report_rml'] or '/'), header=r['header'], register=False, **kwargs)
            else:
                new_report = None
