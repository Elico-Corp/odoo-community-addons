...
class landed_cost_position(orm.Model):
    ...
    def _compute_original_qty(self, cr, uid, ids, name, args, context=None):
        ...
        if cost.purchase_order_id:
            pol_ids = pol_pool.search(
                cr, uid, [('order_id', '=', cost.purchase_order_id.id)], context=context)
            pols = pol_pool.browse(cr, uid, pol_ids, context=context)
            res[cost.id] = sum([pol.remaining_qty for pol in pols])
        elif cost.purchase_order_line_id:
            pol = pol_pool.browse(cr, uid, cost.purchase_order_line_id.id, context=context)
            res[cost.id] = pol.remaining_qty
        return res
    ...
    _columns = {
        ...
        'company_id': fields.related(
            ...
            store=True,
            readonly=True),
        'original_qty': fields.function(
            _compute_original_qty,
            type='float',
            digits_compute=dp.get_precision('Purchase Price'),
            string='Original Quantity',
            store=True),
        'adjusted': fields.boolean('Has been adjusted')
      }
...
class purchase_order_line(orm.Model):
    ...
    def _landing_costs(self, cr, uid, ids, name, args, context=None):
        if not ids:
            return {}
        result = {}
    ...
    _columns = {
        ...
        'landing_costs': fields.function(
            _landing_costs,
            digits_compute=dp.get_precision('Account'),
            string='Remaining Landing Costs')

    }

    def compute_remaining(self, cr, uid, data, context=None):
        if 'product_qty' in data:
                    data.update({'remaining_qty': data['product_qty']})

    def create(self, cr, uid, data, context=None):
        self.compute_remaining(cr, uid, data, context=context)
        if 'order_id' in data:
            order = self.pool.get('purchase.order').browse(cr, uid, data['order_id'], context=context)
            if order.state != "draft":
                raise osv.except_osv('Warning' ,'You cannot create a line from a PO which is not in draft')
        return super(purchase_order_line, self).create(cr, uid, data, context=context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        pols = self.browse(cr, uid, ids, context=context)
        for pol in pols:
            if not pol.order_id:
                continue
            if pol.order_id.state != 'draft':
                raise osv.except_osv('Warning' ,'You cannot unlink a line from a PO which is not in draft')
...
class purchase_order(orm.Model):
    ...
    def _prepare_order_line_move(self, cr, uid, order, order_line, picking_id,
        ...
        try:
            res['price_unit'] = order_line.price_unit + (order_line.landing_costs/ order_line.remaining_qty)
